...
const STRINGS = {
  zh: {
    ...
    mode: {
      ...
      demo: "演示",
      live: "链上",
      demoTip: "演示模式 · 数据为示意，不上链",
      liveTip: "链上模式 · 真读 Sepolia · 钱包可签名",
    },
    wallet: {
      noWallet: "未检测到钱包",
      installPrompt: "请先安装 MetaMask",
      connect: "连接钱包",
      connecting: "连接中…",
      disconnect: "断开",
      wrongNetwork: "错误网络",
      switchToSepolia: "切到 Sepolia",
      sepoliaBadge: "Sepolia",
      addressLabel: "地址",
    },
    live: {
      banner: "当前为链上模式。数据从 Sepolia 实时读取。",
      bannerDemo: "当前为演示模式。点右上角「链上」切换到真链。",
      snapshotTitle: "链上快照",
      snapshotBlock: "区块",
      snapshotFetching: "读取中…",
      snapshotError: "读取失败",
      snapshotRetry: "重试",
      borrowersOnchain: "已注册借款人",
      regulatorOnchain: "已设监管者",
      poolOnchain: "已绑定 Pool",
      governorOnchain: "Governor",
      notSet: "未设置",
      computeScoreTitle: "提交一笔真实的 Sepolia 交易",
      computeScoreSub: "调用 computeScore(1) — 触发链上 FHE 运算、跨合约 ACL 桥接、事件 emit。仅需少量测试 ETH 作 gas。",
      computeScoreBtn: "签名并提交",
      computeScoreSubmitting: "签名中…",
      computeScorePending: "区块确认中…",
      computeScoreSuccess: "已上链 ✓",
      viewTx: "查看交易 ↗",
      viewContract: "查看合约 ↗",
      borrowerOnchainNotice: "提示：链上注册的借款人为 {count} 个；演示中的 8 家机构为示意数据。要真授权监管者，需先以 governor 身份注册借款人钱包（需 relayer-sdk 加密 KYC tier）。",
    },
    persona: {
  ...
  en: {
    ...
    mode: {
      ...
      demo: "Demo",
      live: "Live",
      demoTip: "Demo mode · curated mock data, no chain calls",
      liveTip: "Live mode · real Sepolia reads · wallet-signed writes",
    },
    wallet: {
      noWallet: "No wallet detected",
      installPrompt: "Install MetaMask to connect",
    ...
    live: {
      banner: "Live mode. State is read directly from Sepolia.",
      bannerDemo: "Demo mode. Flip to 'Live' (top-right) to read real Sepolia state.",
      snapshotTitle: "On-chain snapshot",
      snapshotBlock: "Block",
      snapshotFetching: "Fetching…",
      snapshotError: "Failed to fetch",
      snapshotRetry: "Retry",
      borrowersOnchain: "Borrowers registered",
      regulatorOnchain: "Regulator set",
      poolOnchain: "Pool wired",
      governorOnchain: "Governor",
      notSet: "not set",
      computeScoreTitle: "Submit a real Sepolia transaction",
      computeScoreSub: "Calls computeScore(1) — triggers on-chain FHE arithmetic, cross-contract ACL bridge, and event emission. Just needs a little testnet ETH for gas.",
      computeScoreBtn: "Sign & submit",
      computeScoreSubmitting: "Signing…",
      computeScorePending: "Awaiting confirmation…",
      computeScoreSuccess: "Confirmed ✓",
      viewTx: "View transaction ↗",
      viewContract: "View contract ↗",
      borrowerOnchainNotice: "Note: {count} borrowers are registered on chain; the 8 institutions in the demo are illustrative. A real grantRegulatorAccess flow requires governor-side borrower registration (encrypted KYC tier via relayer-sdk).",
    },
    persona: {
